...
i18n + SEO foundation is live (merge `54fa35b`). Start with the **tour-pages `is_published` content-leak gate** (task 1 — small, do first), then the content/photo sprint, then the **French content migration** (Option B draft→review→promote against the live `translations` columns), then CCC v3 **Phase 3**. Housekeeping: regenerate `lib/supabase.types.ts` and tune the seo-audit French title-length rule.
